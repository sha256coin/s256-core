...
static bool ParseArgs(NodeContext& node, int argc, char* argv[])
{
    ...
    // If Qt is used, parameters/sha256coin.conf are parsed in qt/bitcoin.cpp's main()
    ...
    for (int i = 1; i < argc; i++) {
        if (!IsSwitchChar(argv[i][0])) {
            return InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see sha256coind -h for a list of options.", argv[i])));
        }
    }
    return true;
}
...
static bool ProcessInitCommands(ArgsManager& args)
{
    ...
    if (HelpRequested(args) || args.GetBoolArg("-version", false)) {
        ...
        if (args.GetBoolArg("-version", false)) {
            ...
            strUsage += "\n"
                "The " CLIENT_NAME " daemon (sha256coind) is a headless program that connects to the SHA256Coin network to validate and relay transactions and blocks, as well as relaying addresses.\n\n"
                "It provides the backbone of the SHA256Coin network and its RPC, REST and ZMQ services can provide various transaction, block and address-related services.\n\n"
...
static bool AppInit(NodeContext& node)
{
    ...
    try
    {
        // -server defaults to true for sha256coind but not for the GUI so do this here
...
MAIN_FUNCTION
{
    ...
    // Connect sha256coind signal handlers
    noui_connect();
